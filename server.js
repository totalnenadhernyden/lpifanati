...
app.get('/caps', async (req, res) => {
  try {
    const r = await fetch('https://eagri.cz/public/app/wms/plpis.fcgi?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetCapabilities');
    const text = await r.text();
    res.set('Content-Type', 'text/plain');
    res.send(text);
  } catch (err) {
...
app.listen(PORT, () => console.log(`LPIS proxy v3 listening on port ${PORT}`));
